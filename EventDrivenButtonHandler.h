...
enum class ClickType {
  ...
  LONG_CLICK_DOWN,   // Triggered when the button is held long (when reaching long click threshold)
 ...
 *
 * This class enables detection of various button interactions (short/long clicks)
 * and allows registration of callback functions for these events.
 */
class EventDrivenButtonHandler {
 public:
  /**
   ...
   * @param activeLow Specifies if the button operates in active-low mode (default: true)
   * @param pullup Enables the internal pullup resistor (default: true)
   */
  EventDrivenButtonHandler(uint8_t pin, bool activeLow = true, bool pullup = true);
   ...
   * @param threshold Time in milliseconds after which a press is recognized as a valid click
   *
   * @note Difference from setDebounceTime:
   * - clickThreshold: Minimum press duration to be considered a valid click
   * - debounceTime: Minimum time between state changes to filter out mechanical bouncing
   */
  void setClickThreshold(unsigned long threshold);

  /**
   * @brief Sets the threshold for long click detection
   * @param threshold Time in milliseconds after which a press is recognized as a long click
   */
  void setLongClickThreshold(unsigned long threshold);

  /**
   * @brief Sets the debounce time for button state changes
   * @param time Time in milliseconds to ignore state changes after a detected change
   *
  ...
  /**
   * @brief Processes button state changes - must be called regularly in the main loop
   */
  void update();
 ...
 private:
  ...
  std::vector<std::function<void(ClickType)>> handlers;  // List of registered callback functions
